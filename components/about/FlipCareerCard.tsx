  ...
  return (
    ...
    >
      <div className="front absolute w-full h-full flex flex-col justify-center rounded-xl shadow-md text-base px-3 pt-2 pb-3 sm:px-6 sm:pt-2 sm:pb-6 text-darkBlue bg-lightBlue backface-hidden">
        <div className="flex items-center justify-center text-m font-bold text-darkBlue h-1/3 whitespace-pre-line">
          {formattedTitle}
        </div>
        <div className="text-s sm:text-md text-left leading-4 sm:leading-5 flex-grow text-darkBlue">
          {details.map((line, i) => (
            <p key={i} className="mb-2">{line}</p>
          ))}
        </div>
        <div className="text-s sm:text-md text-[#999] italic">{date}</div>
      </div>
